...
def get_component_details(raw: str):
    ...
    - Always returns a dict; useful fields:
        ...
        - chips: list of tapable chip dicts (id, text, price, type, meta) when available
    ...
    try:
        ...
        if callable(lookup_fn):
            try:
                ...
                logger.exception(
                    "lookup_component_by_chip_id raised; continuing to fuzzy lookup.")

        if direct:
            logger.debug("LOOKUP: direct lookup_component_by_chip_id -> %r (%s)",
                         direct.get("name"), direct.get("type"))
            # create tapable chip for the direct match
            chip_type = direct.get("type") or None
            chips = build_chips([direct], chip_type) if chip_type else []
            return {
                "found": True,
        ...
        if best_obj and best_type:
            ...
            # provide a tapable chip for the best-match result so frontend can show details
            chips = build_chips([best_obj], best_type)
            return {
                "found": True,
        ...
        if scores:
            ...
            # Try to convert suggestions into real dataset objects when possible (so we can make chips)
            for s in suggestions:
                try:
                    ...
                    if ctype and name:
                        for comp in (data.get(ctype, {}) or {}).values():
                            if (comp.get("name") or "").strip().lower() == name.strip().lower():
                                suggestion_chips.append(comp)
                                break
                except Exception:
                    continue
        ...
        if suggestions:
            ...
            chips_for_suggestions = build_chips(
                suggestion_chips, suggestion_chips[0].get("type")) if suggestion_chips else []
            return {
                "found": False,
                "error": f"I couldn't find an exact match for '{raw}', but here are close matches.",
                "suggestions": suggestions,
                "chips": chips_for_suggestions,
